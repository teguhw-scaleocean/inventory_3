...
class CheckInPalletDetailScreen extends StatefulWidget {
  final CheckInModel? checkInModel;
  final String? scanBarcode;

  const CheckInPalletDetailScreen(
      {super.key, this.checkInModel, this.scanBarcode = ""});

  @override
  State<CheckInPalletDetailScreen> createState() =>
      _QualityControlDetailScreenState();
}

class _QualityControlDetailScreenState
    extends State<CheckInPalletDetailScreen> {
  ...
  void initState() {
    ...
    cubit = BlocProvider.of<CheckInCubit>(context);

    if (widget.checkInModel != null) {
      checkInModel = widget.checkInModel!;
      debugPrint(checkInModel.toJson());
    }

    if (checkInModel.packageStatus
        .toString()
        .toLowerCase()
      ...
      // idTracking = 2;
      // _scannedFromList();
    } else if (checkInModel.packageStatus
        .toString()
        .toLowerCase()
        .contains("lots")) {
      ...
      cubit.getInitPalletListSn();
      _scannedFromList();
    }

    date = checkInModel.dateTime.substring(0, 10);
    time = checkInModel.dateTime.substring(13, 18);
  }
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: CustomAppBar(
            onTap: () => Navigator.pop(context), title: "Check-in Detail"),
        body: ListView(
          children: [
            Container(
                  ...
                  BlocBuilder<ProductMenuProductDetailCubit,
                    ...
                    return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          tracking = checkInModel.packageStatus.substring(10);
